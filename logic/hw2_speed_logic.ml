...
module Enhanced_game_state = struct
  ...
  let make_move enhanced_state (move : Move.t) (player : string) : (t, string) Result.t =
    ...
    | Ok new_base_state ->
        ...
        | Move.Draw_cards -> 
            Printf.sprintf "%s drew cards" player
        in
        ...
        let win_msg = 
          if new_base_state.game_over then
  ...
  let get_all_moves enhanced_state (player : string) : Move.t list =
    if enhanced_state.base_state.game_over then
      []
    else
      let current_hand = 
        match player with
  ...
  let are_both_players_stuck enhanced_state =
    let player1_can_play = List.exists enhanced_state.base_state.player1_hand ~f:(fun card ->
      Card.can_play_on card enhanced_state.base_state.pile1 ||
      Card.can_play_on card enhanced_state.base_state.pile2
    ) in
    let player2_can_play = List.exists enhanced_state.base_state.player2_hand ~f:(fun card ->
      Card.can_play_on card enhanced_state.base_state.pile1 ||
      Card.can_play_on card enhanced_state.base_state.pile2
    ) in
  ...
  let refresh_center_cards enhanced_state =
    ...
    let new_base_state = {
      ...
      pile1 = Some new_card1;
      pile2 = Some new_card2;
    } in
    
    let refresh_msg = Printf.sprintf "🔄 Center cards refreshed: %s, %s" 
      (Card.to_string new_card1) (Card.to_string new_card2) in
  ...
  let check_for_immediate_win enhanced_state =
    let player1_won = List.is_empty enhanced_state.base_state.player1_hand && 
                     List.is_empty enhanced_state.base_state.player1_stock in
    let player2_won = List.is_empty enhanced_state.base_state.player2_hand && 
                     List.is_empty enhanced_state.base_state.player2_stock in
    
    if player1_won then
      let win_msg = "🎉 Player 1 wins! Player 1 ran out of cards! 🎉" in
      Some {
        enhanced_state with
        base_state = { enhanced_state.base_state with game_over = true; winner = Some Player.Player1 };
        game_log = win_msg :: enhanced_state.game_log;
      }
    else if player2_won then
      let win_msg = "🎉 Player 2 wins! Player 2 ran out of cards! 🎉" in
      Some {
        enhanced_state with
        base_state = { enhanced_state.base_state with game_over = true; winner = Some Player.Player2 };
        game_log = win_msg :: enhanced_state.game_log;
      }
    else
      None
...
let make_enhanced_move enhanced_state move player = Enhanced_game_state.make_move enhanced_state move player
let get_enhanced_moves enhanced_state player = Enhanced_game_state.get_all_moves enhanced_state player
let are_players_stuck enhanced_state = Enhanced_game_state.are_both_players_stuck enhanced_state
...
let example_move = Move.Play_card { card = { Card.suit = Card.Hearts; rank = Card.Ace }; pile = 0 }

...
let heuristic_value (node : Game_state.t) =
  ...
  | None ->
    if node.game_over
    then 0 (* Game over but no winner (shouldn't happen in Speed) *)
    else (
...
let enhanced_heuristic_value (node : Game_state.t) =
  ...
  | None ->
    ...
    then 0
      ...
      in
      let playable_cards =
        List.count current_hand ~f:(fun card ->
          Card.can_play_on card node.pile1 || Card.can_play_on card node.pile2)
      in
...
let rec alpha_beta (node : Game_state.t) depth alpha beta =
  ...
  then enhanced_heuristic_value node
  else (
        ...
        then None
          ...
          let best_move =
            ...
            | Player.Player1 ->
              ...
              List.max_elt moves_and_values ~compare:(fun (_, v1) (_, v2) ->
                Int.compare v1 v2)
              |> Option.map ~f:fst
            | Player.Player2 ->
              (* Minimizing player - choose move with lowest value *)
              List.min_elt moves_and_values ~compare:(fun (_, v1) (_, v2) ->
                Int.compare v1 v2)
              |> Option.map ~f:fst
          in
          best_move)))
...
module AIStrategy = struct
  ...
  let hybrid_strategy enhanced_state =
    ...
    if List.is_empty available_moves
    then None
    else if (* Use alpha-beta for deeper analysis when few moves available *)
            List.length available_moves <= 3
    then alpha_beta_strategy enhanced_state ~depth:3
    else greedy_strategy enhanced_state
  ;;
  ...
  let adaptive_strategy enhanced_state =
    ...
    then None
      ...
      let player2_cards =
        List.length enhanced_state.player2_hand + List.length enhanced_state.player2_stock
      in
      let player1_cards =
        List.length enhanced_state.player1_hand + List.length enhanced_state.player1_stock
      in
      (* If we're behind, use deeper search *)
      if player2_cards > player1_cards + 5
      then
        alpha_beta_strategy enhanced_state ~depth:4
        (* If we're ahead, use faster greedy *)
      else if player2_cards < player1_cards - 5
      then greedy_strategy enhanced_state (* Otherwise, use moderate search *)
      else alpha_beta_strategy enhanced_state ~depth:2)
  ;;
end
...
let ai_move enhanced_state = ai_choose_move enhanced_state ~strategy:default_ai_strategy

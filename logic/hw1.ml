...
module Card = struct
  ...
  let can_play_on card pile_card =
    ...
    then false
      ...
      if Poly.(card.rank = Ace)
      then pile_val = 13 || pile_val = 2 (* King or 2 *)
      else if Poly.(pile_card.rank = Ace)
      then card_val = 13 || card_val = 2 (* King or 2 *)
      else (* Normal ±1 rule *)
...
module Game_state = struct
  ...
  let draw_cards player game_state =
    ...
    if cards_to_draw = 0
    then Error Move_error.No_cards_to_draw
    else (
  ...
  let make_move game_state (move : Move.t) : (t, Move_error.t) Result.t =
    ...
    then Error Move_error.Game_is_over
    else (
      match move with
      | Move.Draw_cards -> draw_cards game_state.current_player game_state
      | Move.Play_card { card; pile } ->
        ...
        if not (List.mem current_hand card ~equal:Card.equal)
        then Error Move_error.Card_not_in_hand
          ...
          in
          if pile < 0 || pile > 1
          then Error Move_error.Empty_pile
          else if (* Check if card can be played *)
                  not (Card.can_play_on card pile_card)
          then Error Move_error.Invalid_play
            ...
            (* Check for winner *)
            let winner =
              if has_won Player.Player1 new_game_state
              then Some Player.Player1
              else if has_won Player.Player2 new_game_state
              then Some Player.Player2
              else None
            in
            let final_game_state =
              { new_game_state with game_over = Option.is_some winner; winner }
            in
            Ok final_game_state)))
  ;;
  ...
  let to_string game_state =
    let hand_to_string hand = List.map hand ~f:Card.to_string |> String.concat ~sep:" " in

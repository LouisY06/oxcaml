...
module SpeedCardTests = struct
  let test_card_creation () =
    ...
    assert_equal
      (Hw1.Card.equal card { Hw1.Card.suit = Hw1.Card.Hearts; rank = Hw1.Card.Ace })
  ;;
...
module SpeedPlayerTests = struct
  let test_player_opposite () =
    assert_equal
      (Hw1.Player.equal (Hw1.Player.opposite Hw1.Player.Player1) Hw1.Player.Player2);
    assert_equal
      (Hw1.Player.equal (Hw1.Player.opposite Hw1.Player.Player2) Hw1.Player.Player1)
  ;;
end
...
module SpeedGameStateTests = struct
  ...
  let test_illegal_card_play_invalid_sequence () =
    ...
    | Ok state_after_first ->
      ...
      let incompatible_card =
        List.find_exn player2_hand ~f:(fun card ->
  ...
  let test_draw_cards_legal () =
    ...
    let modified_state =
      { game_state with player1_hand = List.take game_state.player1_hand 3 }
    in
    ...
    | Ok new_state ->
      ...
      assert_equal (Hw1.Player.equal new_state.current_player Hw1.Player.Player2);
      assert_equal
        (List.length new_state.player1_stock < List.length modified_state.player1_stock)
    | Error _ -> assert_equal false
  ;;
  ...
  let test_get_all_moves () =
    ...
    assert_equal
      (List.exists moves ~f:(function
         | Hw1.Move.Play_card _ -> true
         | _ -> false))
  ;;
end
...
module RandomExplorationTests = struct
  ...
  let test_random_speed_game_play () =
    ...
    while (not !game_state.game_over) && !move_count < max_moves do
      ...
      then game_state := { !game_state with game_over = true }
      else (
  ...
  let test_random_card_game_stress () =
    ...
    for _ = 1 to total_tests do
      ...
      while
        (not !game_state.game_over) && !move_count < max_moves && not !game_completed
      do
        let moves = Hw1.Game_state.get_all_moves !game_state in
        if List.is_empty moves
        then (
          ...
          | Ok new_state ->
            game_state := new_state;
            move_count := !move_count + 1
          | Error _ -> game_completed := true)
      done;
      if !game_state.game_over || !move_count >= max_moves
...
module IntegrationTests = struct
  ...
  let test_edge_case_empty_hands () =
    let game_state = Hw1.Game_state.create () in
    let empty_hand_state = { game_state with player1_hand = []; player1_stock = [] } in
    (* Player1 should have won *)
    let player1_won =
      List.is_empty empty_hand_state.player1_hand
      && List.is_empty empty_hand_state.player1_stock
    in
  ...
  let test_edge_case_invalid_pile_number () =
    ...
    let player1_hand = game_state.player1_hand in
    let first_card = List.hd_exn player1_hand in
    let move = Hw1.Move.Play_card { card = first_card; pile = 5 } in
    (* Invalid pile *)
    match Hw1.Game_state.make_move game_state move with
    | Ok _ -> assert_equal false

...
let apply_action (action : Action.t) (model : Model.t) =
   ...
   | Play_card (card, pile_index) ->
      ...
      (match
       ...
       | Error _msg -> model) (* Ignore invalid moves for now *)
   | AI_move ->
   ...
   let card_to_html card is_selected is_player_card ~inject =
      ...
      Node.div
         ~attrs:
            ...
            ; Attr.create "data-suit"
                 (match card.Hw2_speed_logic.Card.suit with
                  | Hw2_speed_logic.Card.Hearts -> "hearts"
                  | Hw2_speed_logic.Card.Diamonds -> "diamonds"
                  | Hw2_speed_logic.Card.Clubs -> "clubs"
                  | Hw2_speed_logic.Card.Spades -> "spades")
            ; Attr.create "data-rank"
                 (match card.Hw2_speed_logic.Card.rank with
                  | Hw2_speed_logic.Card.Ace -> "ace"
                  | Hw2_speed_logic.Card.Two -> "two"
                  | Hw2_speed_logic.Card.Three -> "three"
                  | Hw2_speed_logic.Card.Four -> "four"
                  | Hw2_speed_logic.Card.Five -> "five"
                  | Hw2_speed_logic.Card.Six -> "six"
                  | Hw2_speed_logic.Card.Seven -> "seven"
                  | Hw2_speed_logic.Card.Eight -> "eight"
                  | Hw2_speed_logic.Card.Nine -> "nine"
                  | Hw2_speed_logic.Card.Ten -> "ten"
                  | Hw2_speed_logic.Card.Jack -> "jack"
                  | Hw2_speed_logic.Card.Queen -> "queen"
                  | Hw2_speed_logic.Card.King -> "king")
            ]
         [ Node.text (Hw2_speed_logic.Card.to_string card) ]
   ...
   let view (model : Model.t) (inject : Action.t -> unit Effect.t) =
         ...
         Node.div
            ~attrs:
               ...
               ; on_click (fun _ ->
                     ...
                     | Some card ->
                          inject (Action.Play_card (card, (if String.equal pile_id "pile1" then 0 else 1)))
                     | None -> Effect.Ignore)
               ]
            [ Node.div ~attrs:[ class_ "pile-label" ]
                 [ Node.text
                      (Printf.sprintf "Center Pile %s"
                         (String.sub pile_id ~pos:4 ~len:1))
                 ]
            ; (match pile_card_opt with
               | Some card -> card_to_html card false false ~inject
      ...
      Node.div
         ...
         [ Node.div
              ...
              ; Node.div ~attrs:[ class_ "game-status"; id "gameStatus" ]
         ...
         ; Node.div
              ...
              [ Node.div ~attrs:[ class_ "info-item" ]
              ...
              ; Node.div ~attrs:[ class_ "info-item" ]
                   ...
                   ; Node.text
                        " Cards must be ±1 rank from pile top. Aces are wild (King or 2)."
                   ]
              ; Node.div ~attrs:[ class_ "info-item" ]
